...
import { FaFacebookF, FaTwitter, FaLinkedinIn, FaInstagram, FaArrowRight, FaArrowUp } from 'react-icons/fa';

const Footer = () => {
  // 👇 STATE: Arrow show karna hai ya nahi
  const [showArrow, setShowArrow] = useState(false);

  // 👇 SCROLL EVENT: Check karega ke user bilkul end par hai ya nahi
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Agar scroll position document ke end se 150px pehle hai, toh arrow dikhao
    ...
    <footer className="bg-[#0f0f0f] text-white font-manrope relative">
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
          ...
          <div className="max-w-lg flex flex-col gap-4">
            <p className="text-[#9B9B8A] text-sm leading-relaxed">
              TransNova Solutions LLC is a global business solutions provider committed to innovation, integrity, and impact.
            </p>
            <Link to="/about" className="text-[#FF6B35] text-sm font-medium flex items-center gap-2 hover:gap-3 transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center mt-2">
              <input type="email" placeholder="Enter your email" className="bg-[#1a1a1a] border border-[#2a2a2a] text-white text-sm px-4 py-2.5 rounded-l-md w-full focus:outline-none focus:border-[#FF6B35]" />
      ...
      <div className="border-t border-[#2a2a2a] py-8 px-6 lg:px-12 relative">
        ...
        <button 
          onClick={scrollToTop}
          className={`
            mx-auto block bg-[#FF6B35] text-white w-12 h-12 rounded-full flex items-center justify-center shadow-lg shadow-orange-500/30 hover:bg-[#E85C2D] transition-all duration-300 hover:-translate-y-1 mb-4
        ...
        {/* Copyright Text */}
        <p className="text-center text-[#9B9B8A] text-xs">
